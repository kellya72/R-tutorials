...
**Exercise: Using the `californiaFires` data. Seperate the `damage` variable into two separate variables: `acresBurned` and `costOfDamage`.** 

```{r ex_download_spread, exercise=TRUE, exercise.eval= FALSE}
californiaFiresData1 <- californiaFiresData

head(californiaFiresData)
```
```{r ex_download_spread-solution}
# californiaFiresData <- read.csv("californiaFires.csv")
californiaFiresData1 <- californiaFiresData %>% 
                          separate(damage, into = c("acresBurned", "costOfDamage"), sep = "/")
...
**Exercise: Using the data frame created in Exercise 1, split the `costOfDamage` into `millions` and `thousands`.**

```{r ex_separate, exercise=TRUE, exercise.eval= FALSE}
californiaFiresData2 <- californiaFiresData1

head(californiaFiresData2)
```
```{r ex_separate-solution}
californiaFiresData2 <- californiaFiresData1  %>% 
                          separate(costOfDamage, into = c("millions", "thousands"), sep = -6)
head(californiaFiresData2)
```
...
```{r ex_unite, exercise=TRUE, exercise.eval= FALSE}
